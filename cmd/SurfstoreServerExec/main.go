...
func startServer(hostAddr string, serviceType string, blockStoreAddrs []string) error {
	...
	if serviceType != "block" {
		metas := surfstore.NewMetaStore(blockStoreAddrs)
		surfstore.RegisterMetaStoreServer(grpcServer, metas)
	}

	if serviceType != "meta" {
		blocks := surfstore.NewBlockStore()
		surfstore.RegisterBlockStoreServer(grpcServer, blocks)
	}
